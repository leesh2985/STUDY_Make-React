...
export const fetchPokemonsAPI = async (nextUrl?: string) => {
  const requesUrl = nextUrl ? nextUrl : "https://pokeapi.co/api/v2/pokemon";
  // post, get, put, delete

  const response = await remote.get<PokemonListReponseType>(requesUrl);

  return response.data;
};

interface PokemonDetailReponseType {
  id: number;
  weight: number;
  height: number;
  name: string;
...
interface PokemonSpeciesReponseType {
  color: {
    name: string;
  };
  names: {
...
export const fetchPkoemonDetailAPI = async (
  name: string
  ...
  const response = await remote.get<PokemonDetailReponseType>(pokemonDetailUrl);
  const speciesResponse = await remote.get<PokemonSpeciesReponseType>(
    pokemonSpeciesUrl
  );
